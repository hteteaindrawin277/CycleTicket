...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:123@localhost:5432/CycleTicket'
...
def ticket_order():
    ...
    if len(form_selected_tickets):
        total_price = len(form_selected_tickets) * price
        selected_ticket = ""
        ...
        if session and session['username']:
            ...
            if validUser:
                ...
                order_id = order.id
                return render_template('order.html', username=validUser, tickets=form_selected_tickets,
                                       totalprice=total_price, price=price, orderId=order_id)
            else:
...
def check(checkid):
    if session and session['username']:
        ...
        if data.verify == 'Rejected':
            ...
            orderId = checkid
            return render_template('order.html', username=validate_user, checking=data.verify, data=data,
                                   pending_tickets=pending_tickets, price=price, total_price=total_price, oid=orderId)
        ...
        for order in orders:
            if order.image:
                ...
                if not order.verify:
                    ...
                    print(order_pending)
        return render_template('admin_dashboard.html', order_tickets=order_tickets, admin_name=valid_user,
